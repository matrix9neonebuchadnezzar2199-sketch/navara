...
description: 拡張性の高い汎用 3D 地図エンジン Navara の概要。
sidebar:
  order: 1
---
...
実際に試してみたい場合は、[Getting Started](../getting-started/) でプロジェクトのセットアップ手順を追えます。また、[Examples](https://navara.world/examples/) ギャラリーでは Navara で何ができるかをブラウザ上のライブデモで確認できます。
...
ビルトインで足りない場合は、独自のメッシュ・エフェクト・ライトの [Descriptor](../../../three/core/custom-desc/) を作成できます。深度バッファや法線 / G バッファ（MRT）を含む、レンダリングエンジンのシーングラフとレンダーパイプラインへのフルアクセスを持ちます。これは Navara のビルトイン Descriptor を支えるのと同じ基盤であり、制限された抜け道ではありません。
