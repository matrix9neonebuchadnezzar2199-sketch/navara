...
type SiteHeaderProps = {
  ...
  /** Accessible label for the language picker (localized by the caller). */
  langLabel: string;
};
...
export const SiteHeader = ({ lang, setLang, langLabel }: SiteHeaderProps) => {
  return (
    <header className="sticky top-0 z-10 bg-background/60 backdrop-blur-md">
      {/* Full-width row like the LP header: brand at the left edge, nav at
      ...
      <div className="flex items-center justify-between px-12 py-5 max-[960px]:px-7 max-[960px]:py-[18px] max-[640px]:px-5 max-[640px]:py-3.5">
        ...
        <nav className="flex items-center gap-7 text-sm font-medium max-[640px]:gap-[18px]">
          ...
          <a
            ...
            aria-label="GitHub"
          >
            {/* GitHub invertocat, matching the LP header's inlined icon. */}
            <svg
          ...
          <LangSelect lang={lang} setLang={setLang} label={langLabel} />
        </nav>
      </div>
    </header>
  );
