...
- `picking: true` (constructor, default on) is required for the `featureClick` (click) and `featureHover`/`featureEnter`/`featureLeave` (mousemove) events and pickable meshes. Hover picking activates only while a hover-family listener is registered.
